...
namespace driver {
	...
	NTSTATUS close(PDEVICE_OBJECT device_object, PIRP irp) {
		...
		if (target_process != nullptr) {
			...
			debug_print("App closed: Target process dereferenced safely.");
		}
...
void driver_unload(PDRIVER_OBJECT driver_object) {
	...
	if (driver::target_process != nullptr) {
		ObDereferenceObject(driver::target_process);
...
NTSTATUS DriverInitialize(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	debug_print("Real Driver Initialization called.");
...
extern "C" NTSTATUS DriverEntry(PDRIVER_OBJECT driver_object, PUNICODE_STRING registry_path) {
	...
	debug_print("DriverEntry called from kdmapper.");

	UNICODE_STRING driver_name = RTL_CONSTANT_STRING(L"\\Driver\\NXWire");
